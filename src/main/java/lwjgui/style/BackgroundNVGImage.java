...
public class BackgroundNVGImage extends Background {
	...
	public static BackgroundNVGImage fromOffscreenBuffer(Window window, OffscreenBuffer buffer) {
		if ( window.getContext().isModernOpenGL() ) {
			return new BackgroundNVGImage(NanoVGGL3.nvglCreateImageFromHandle(window.getContext().getNVG(), buffer.getColorTexId(), buffer.getWidth(), buffer.getHeight(), NanoVG.NVG_IMAGE_FLIPY));
		} else {
			return new BackgroundNVGImage(NanoVGGL2.nvglCreateImageFromHandle(window.getContext().getNVG(), buffer.getColorTexId(), buffer.getWidth(), buffer.getHeight(), NanoVG.NVG_IMAGE_FLIPY));
		}
	}
