...
public class BlurPane extends StackPane {
	...
	protected void init() {
		super.init();
		bufferTemp = new OffscreenBuffer((int)oldSize.x, (int)oldSize.y, false);
		buffer = new BlurBuffer(bufferTemp.getWidth(), bufferTemp.getHeight(), bufferTemp);
		if ( this.window.getContext().isModernOpenGL() ) {
			nanoImage = NanoVGGL3.nvglCreateImageFromHandle(this.window.getContext().getNVG(), buffer.getColorTexId(), oldSize.x, oldSize.y, NanoVG.NVG_IMAGE_FLIPY);
		} else {
			nanoImage = NanoVGGL2.nvglCreateImageFromHandle(this.window.getContext().getNVG(), buffer.getColorTexId(), oldSize.x, oldSize.y, NanoVG.NVG_IMAGE_FLIPY);
		}
	}
	...
	class BlurBuffer extends OffscreenBuffer {
		...
		public void render(Context context, int x, int y, int w, int h) {
			...
			if ( this.quad == null || quadDirty ) {
				...
				quad = new TexturedQuad(0, 0, w, h, source.getColorTexId());
			}
			...
			GL11.glBindTexture(GL11.GL_TEXTURE_2D, source.getColorTexId());
